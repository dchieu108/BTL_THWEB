        ...
        if(isset($_SESSION["giohang"])) {
            foreach($_SESSION["giohang"] as $value) {
                if ($_POST["so_luong".$value["ID"]] <=0) {
                    unset($_SESSION["giohang"][$value["ID"]]);
                }
                else{
                    $_SESSION["giohang"][$value["ID"]]["so_luong"] = $_POST["so_luong".$value["ID"]];
                }
            }
        }
    }
    if(isset($_POST["order"])) {
        if(empty($_POST["customer"]) || empty($_POST["phone"]) || empty($_POST["address"]) || empty($_POST["total"]))  {
            echo 0;
        }
        else {
            $giohang = !empty($_SESSION["giohang"])?$_SESSION["giohang"]:[];
            $customer = $_POST["customer"];
            $phone = $_POST["phone"];
            $address = $_POST["address"];
            $total = $_POST["total"];
            $sql = "INSERT INTO `donhang`(`ten_khach_hang`, `so_dien_thoai`, `dia_chi`, `tong_tien`) VALUES ('$customer','$phone','$address','$total')";
            
            $query = $connect->query($sql);
            $order = "SELECT MAX(ID) as 'ID' FROM donhang";
            $order_query = mysqli_query($connect, $order);
            while ($id_donhang = mysqli_fetch_array($order_query)) {
                $id_order = $id_donhang["ID"];
            }
            foreach ($giohang as $value) {
                $id = $value["ID"];
                $sl = $value["so_luong"];
                $order_detail = "INSERT INTO `chi_tiet_don_hang`(`id_hang_hoa`, `so_luong`, `id_don_hang`) VALUES ('$id','$sl','$id_order')";
                $result_detail = mysqli_query($connect, $order_detail);
            }
...
<body>
 ...
 <!-- end header -->                                                    
    ...
    <div>
                ...
                <?php 
                    ...
                    if (isset($_SESSION["giohang"])) {
                        ...
                        <tr>
                            ...
                            <td><?php echo $tong; ?></td>
                            <td><a href="delete.php?id=<?php echo $value["ID"];?>" class="no-underline">Xóa</a></td>
                        </tr>
                <?php            
        ...
        <div class="dh">
            <p>Tổng Giá Trị Đơn Hàng:<?php echo number_format($total);?></p>
            <button type="submit" name="update" >Cập Nhật Đơn Hàng</button>
        </div>
        <div class="form-group">
           <h2>Thông tin đặt hàng</h2>
        </div>
        <div class="form-group">
            <input type="text" name="customer" placeholder="Nhập Họ Tên">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            <input type="submit" name="order" value="Đặt Đơn Hàng">
        </div>
