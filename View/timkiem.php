        ...
        <div class="product-men ">
            ...
            <div class="list-product d-flex w-70">
                            ...
                            if (isset($_POST['keyword'])) {
                                ...
                                $sql = "SELECT ID, Anh, TenSanPham, Gia FROM sanpham WHERE TenSanPham LIKE '%$keyword%'";
                                $result = $connect->query($sql);
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        $Gia_formatted = number_format($row['Gia'], 0, ',', '.');
                                        echo '<div class="product-item w-25 position-rel ">
                                        <div class="product-cate position-rel">
                                            <div><img src="../Images/' . $row['Anh'] . '" class="w-100"></div>
                                            <div class="product-sale">20%</div>
                                        </div>
                                        <div class="product-name text-center">
                                            <h4>KASHMIR</h4>
                                            <a href="chitietsanpham.php?id=<?php echo $value["ID"];?><p>' . $row['TenSanPham'] . '</p></a>
                                            <p>
                                                <a class="fw-900" href="chitietsanpham.php?id=<?php echo $value["ID"];?><p>' . $Gia_formatted . 'đ</p></a>
                                            </p>
                                        </div>
